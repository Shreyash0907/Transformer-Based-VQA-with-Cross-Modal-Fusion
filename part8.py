...
dset_path = ""
savepth = ""
mdl_pth_arg = "/kaggle/input/best-models/best_model_part8.pth"
...
def train_model_main(device, tknzer, img_transfrm):
    ...
    print("Starting Part 8: Training")
    ...
    mdl = Vqa(emb_dim=emb_dim,n_classes=n_classes, vocab_size=vocab_size,mx_len=mx_len, n_heads=n_heads, n_layers=n_layers,  tknzer_pad_id=tknzer.pad_token_id).to(device)
...
def inference_model_main(device, tknzer, img_transfrm):
    ...
    print("\nTest Set A Results (Part 8 - Inference Mode)")
    metrics = {
        "Accuracy": tst_acc,
        "Precision": tst_prec,
        "Recall": tst_recall,
    ...
    print("\nPart 8 inference script finished")

def main():
    parser = argparse.ArgumentParser(description="VQA Model Part 8: Training and Inference")
