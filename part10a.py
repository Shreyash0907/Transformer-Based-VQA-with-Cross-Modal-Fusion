...
# Vqa model
class Vqa(nn.Module):
    def __init__(self, vocab_size, emb_dim, n_heads, n_layers, n_classes, mx_len, tknzer_pad_id, require_grad=False):
        super().__init__()
        ...
        self.text_embedding = self.init_text_embedding(vocab_size, emb_dim, tknzer_pad_id)
    ...
    def init_text_embedding(self, vocab_size, emb_dim, padding_idx):
        return nn.Embedding(vocab_size, emb_dim, padding_idx=padding_idx)
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        if self.alpha != 1:
            ones = torch.ones_like(targets, device=targets.device)
            alpha_weight = ones * self.alpha
            focal_weight = focal_weight * alpha_weight.unsqueeze(1)
        prob = log_probs.gather(1, targets.unsqueeze(1))
        focal_loss = (-focal_weight) * prob
...
def train_model_main(device, tknzer, img_transfrm):
    ...
    print("Starting Part 10a: Training")
    ...
    mdl = Vqa(emb_dim=emb_dim,n_classes=n_classes, vocab_size=vocab_size,mx_len=mx_len, n_heads=n_heads, n_layers=n_layers,  tknzer_pad_id=tknzer.pad_token_id, require_grad=True).to(device)

    critrn = FocalLoss(alpha=1.0, gamma=2.0)
...
def inference_model_main(device, tknzer, img_transfrm):
    ...
    print("\nTest Set A Results (Part 10a - Inference Mode)")
    metrics = {
        "Accuracy": tst_acc,
        "Precision": tst_prec,
        "Recall": tst_recall,
    ...
    print("\nPart 10a inference script finished")

def main():
    parser = argparse.ArgumentParser(description="VQA Model Part 10a: Training and Inference")
